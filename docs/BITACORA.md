...
## 2026-09-21 — F7.9: `/arrival-guide`, dos bugs de extracción de la home destapados de paso, y un bug de layout real

La primera página del sitio además de la home. D-P6 ya venía resuelta desde antes de esta tarea: el video de la referencia muestra a *su* anfitriona y *su* letrero — el prototipo ya lo había reemplazado por un poster propio (`sprinter-interior.jpg`, F0 cierre), así que no hubo nada que filtrar, solo portar. En esta página no existe ningún `<video>`.

**Un extractor con nombre por sección, no el molde genérico de la home.** `extract-home.mjs` trata sus 26 secciones como filas de una misma tabla (párrafos, listas, CTA, imágenes) porque de verdad tienen la misma forma. Las cinco de `arrival-guide.html` no: hero, "cómo encontrarnos", aviso, pasos numerados y CTA final son estructuras distintas entre sí. Forzarlas al molde genérico habría significado más código de acomodo que el que ahorra reusarlo. Se escribió `extract-arrival-guide.mjs` aparte, con un objeto de salida con nombre por sección — y lo que de verdad era común entre los dos extractores (limpiar entidades, texto con o sin `<strong>/<em>`, separar `<body>` en `<section>`) se movió a `scripts/lib/prototype-html.mjs`, la primera vez que un helper de estos pasa de un archivo a compartido.

**Portar una página nueva destapó dos bugs silenciosos en la que ya estaba en producción.** Ninguno de los dos rompía nada visible en la home —por pura coincidencia de qué secciones tiene cada página—, así que llevaban ahí desde F7.6 sin que ninguna prueba los atrapara:
1. `<p[^>]*>` y `<li[^>]*>`, sin límite de palabra, hacen match con el arranque de cualquier otra etiqueta que empiece con esa letra — un `<path>` de ícono SVG, por ejemplo — y el `.*?` no goloso sigue de largo hasta el primer `</p>` real, tragándose lo que hay en medio. El ícono de alerta de "Be Aware" en `arrival-guide.html` tiene justo un `<path>` antes de su `<p>` de verdad, y el párrafo salía con "Be Aware" duplicado al principio. En la home el mismo patrón nunca se notó porque ninguna sección con `<path>` cerca de un `<p>` real dejó un resultado tan obvio. Corregido a `<p\b[^>]*>`/`<li\b[^>]*>` en los dos extractores.
2. `splitSections` cortaba el `<body>` en cada `<section`, sin acotar el final — así que la **última** sección se quedaba con todo lo que viene después: el `<footer>` completo, scripts, estilos. En la home la última sección real es el FAQ, cuyo extractor ya descarta párrafos y listas sueltas por su propia cuenta, así que la fuga quedaba encubierta ahí por pura coincidencia. En `arrival-guide.html` la última es el CTA final, sin ese filtro — el pie de página se colaba entero como si fuera su contenido. Se acotó `splitSections` a `body.lastIndexOf("<footer")` — el *último*, no el primero, porque las ocho reseñas de la sección de testimonios traen cada una su propio `<blockquote><footer>` de cita.

Al volver a correr `extract-home.mjs` con las dos correcciones, `home.json` cambió en un solo lugar: el FAQ perdió una CTA y una imagen que en realidad eran del pie de página colado. Se comparó sección por sección contra la versión publicada — sin ninguna otra diferencia.

**Un color de menos, a propósito.** El aviso "Be Aware" del prototipo usa ámbar, un acento ajeno a los dos colores de CTC. En vez de sumar un tercero, se reconstruyó con los mismos tokens de dorado que ya tiene el resto del sitio — gastar el atrevimiento en un solo lugar (§3.5.6), no en cada advertencia. El link interno del CTA final ("dónde está Cabo y por qué está en México") apuntaba a `href="#"` en el prototipo — esa página de geografía es contenido de F9 y no existe todavía — se dejó como texto plano en vez de publicar un enlace que no lleva a ningún lado.

**Un bug de layout real, no de extracción.** El CTA dorado final envolvía "Reserve now" en dos líneas y cortaba la primera a la mitad ("RESE" / "RVE NOW"), con la caja entera midiendo 80×70 px — casi exactamente el ancho de su propio padding horizontal (`px-10` = 80 px), como si no quedara espacio para el texto. Causa: este único CTA llevaba `inline-block` en su clase, a diferencia del CTA del hero y los de `HomeSection` (que son `<a>` simples, sin ese `display`) — combinado con `overflow: hidden` de `.ctc-shine` (F7.8, para recortar el barrido de luz del `::after`), el cálculo de ancho por contenido ("shrink-to-fit") del navegador colapsaba a un tamaño menor que el propio texto. Se quitó `inline-block`, igual que en los demás botones dorados, y el botón volvió a medir lo que su texto necesita (185×48 px).

**Nota para la próxima sesión que depure visualmente con capturas en esta máquina.** El hero de esta página —y, en su momento, también el de la home— se veía completamente negro en las capturas de Playwright, pixel a pixel igual a `--color-deep`, mientras el HTML, el CSS computado y el archivo de imagen servido eran correctos (confirmado leyendo los píxeles del `<img>` ya cargado por `canvas`, *desde dentro* de la página: mostraba sus colores reales, un atardecer con la foto sin ningún problema). Era un bug de rasterizado GPU del Chromium headless de este sandbox, no del sitio — lanzándolo con `--disable-gpu --force-color-profile=srgb` la foto aparece bien. Una hora se fue reescribiendo el degradado del overlay (`from-deep/55 via-deep/70 to-deep/90` → capas de `opacity` + `transparent`) antes de aislar la causa real y revertir ese cambio, que nunca hizo falta. Ninguna captura de este sandbox debe tomarse como diagnóstico visual sin ese flag.

**Archivos:** `web/scripts/lib/prototype-html.mjs` (nuevo), `web/scripts/extract-arrival-guide.mjs` (nuevo), `web/scripts/extract-home.mjs`, `web/src/content/arrival.json` (nuevo), `web/src/content/arrival.ts` (nuevo), `web/src/content/home.json`, `web/src/pages/arrival-guide.astro` (nuevo), `web/public/images/arrival-guide/*` (nuevo), `web/scripts/perf.mjs` (acepta rutas), `web/package.json` (`extract:arrival-guide`, `perf` ahora revisa dos rutas), `web/tests/arrival-guide.spec.ts` (nuevo), `web/tests/home.spec.ts`, `WORKPLAN.md`

**Verificación**
- `npm run e2e` → **58 passed** (6 nuevos): las cinco secciones con su contenido, cero `<video>` en la página, el CTA en una sola línea, contenido completo sin JavaScript, el poster listo para LCP sin depender de JS, y sin desbordamiento horizontal a 390 px.
- `npm run build`, `npm run check` (0/0/0), `format:check`, `npm run seo` (100/100, sobre `/`) y `npm audit` (0 vulnerabilidades): todo limpio.
- `npm run perf` ahora corre contra `/` y `/arrival-guide`. En esta máquina, con el sandbox bajo carga real (decenas de pestañas de Chrome abiertas durante la sesión), midió 4.2-4.3 s y 2.9-3.2 s respectivamente — por encima del presupuesto de 2.5 s, y por encima de los 1.9 s estables que F7.7 había medido para la home con el mismo código de hero. `throttlingMethod: "devtools"` acelera de verdad la red y la CPU *disponibles*, así que un sandbox ocupado por otra cosa infla la medición sin que el sitio haya cambiado — el runner de CI, aislado, es el que decide si esto es una regresión real o ruido de esta máquina.

**F7 queda en 9/16.** Siguiente: F7.10, `lib/api.ts` usando solo `packages/api-client` — que una ruta de API mal escrita no compile.
