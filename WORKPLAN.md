...
| Indicador | Estado |
|---|---|
| **Fase actual** | F6 — Auth y API del admin: 12/13 (login, sesión, TOTP obligatorio para owner/manager, CSRF de doble token, listado y acciones de reservas, alta manual, despacho, flota y cuentas por cobrar, tareas, auditoría automática, dashboard y KPIs, catálogo CRUD). F5 — Emails, PDF y trabajos: 7/11 (falta lo programado: recordatorio 24 h, reseña, aviso al chofer, webhooks de Resend). F4 — Pagos con Stripe: 6/10 (F4.1 a F4.5, F4.9). F2 — Motor de precios y catálogo: 10/12 (falta F2.9, revisión de Marlon, y F2.12). F3 — Reservas públicas: 11/13. F1 ✅ 12/12. F0 ✅ 9/9. Vista previa del prototipo en https://cabotransportationconcierge.vercel.app (noindex). |
...
**Siguiente tarea:** F6.13 (usuarios y roles, solo owner, y lista de auditoría), la última tarea de F6. Su propio criterio de verificación ("un dispatcher no puede editar tarifas, 403") pide afinar el permiso por rol en las rutas de catálogo de F6.12, que hoy comparten el `CAN_EDIT` genérico de F6.5 (cualquier rol menos `viewer`) — F6.13 es quien le pone a `/admin/rates` (y probablemente al resto del catálogo) un permiso más estricto que excluya `dispatcher`. Con eso F6 (auth y API del admin) queda completo. F6 en conjunto desbloquea F4.6 a F4.8 (link de pago, reembolsos, pagos manuales — F6.5 ya cubrió el reembolso y el pago manual sobre una reserva existente; falta el link de pago real de Stripe Checkout, F4.6) y F5.9 (aviso al chofer asignado, ya desbloqueado desde F6.7). F5.7 y F5.8 (recordatorio 24 h y solicitud de reseña) siguen pendientes de una tarea programada. F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales) quedan para cuando se necesiten esos servicios. Para F4.10 hacen falta las llaves de prueba de Stripe del cliente.
...
F4  Pagos con Stripe                [██████----] 6/10
F5  Emails, PDF y trabajos          [██████----] 7/11
F6  Auth y API del admin            [█████████-] 12/13
...
- [x] **F6.12** — Catálogo CRUD (zonas, hoteles, matriz de tarifas, extras, actividades, promociones) y settings. Verificar: tests. Verificado: `test_admin_catalog.py` (alta y edición de cada uno; editar una tarifa deja el `AuditLog` con el diff exacto, cerrando de verdad el criterio de F6.10). Por ahora las siete rutas usan el mismo `CAN_EDIT` de F6.5 (cualquier rol menos `viewer`); F6.13 es quien afina el permiso por rol (un `dispatcher` no debería poder tocar tarifas).
