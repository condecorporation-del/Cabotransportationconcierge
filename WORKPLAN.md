...
| Indicador | Estado |
|---|---|
| **Fase actual** | F6 — Auth y API del admin: 2/13 (login, sesión y TOTP obligatorio para owner/manager). F5 — Emails, PDF y trabajos: 7/11 (falta lo programado: recordatorio 24 h, reseña, aviso al chofer, webhooks de Resend). F4 — Pagos con Stripe: 6/10 (F4.1 a F4.5, F4.9). F2 — Motor de precios y catálogo: 10/12 (falta F2.9, revisión de Marlon, y F2.12). F3 — Reservas públicas: 11/13. F1 ✅ 12/12. F0 ✅ 9/9. Vista previa del prototipo en https://cabotransportationconcierge.vercel.app (noindex). |
...
**Siguiente tarea:** F6.3 (CSRF de doble token y middleware de roles) antes de exponer cualquier endpoint de negocio del admin, ya que F6.1/F6.2 solo dejaron la sesión autenticada sin esa protección. Con eso listo, F6.4 (listado de reservas) es la puerta de entrada al resto de F6. F6 en conjunto desbloquea F4.6 a F4.8 (link de pago, reembolsos, pagos manuales) y F5.9 (aviso al chofer asignado, que hoy no existe sin despacho). F5.7 y F5.8 (recordatorio 24 h y solicitud de reseña) piden una tarea programada, que también conviene resolver junto con el admin. F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales) quedan para cuando se necesiten esos servicios. Para F4.10 hacen falta las llaves de prueba de Stripe del cliente.
