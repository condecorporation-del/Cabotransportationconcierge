...
| **Sitio público real** | 🟡 `web/` en pie (F7.1): Astro 7 en modo estático (SSG) con TypeScript estricto, Tailwind 4 por el plugin de Vite e islas de React 19, más Playwright corriendo contra el build real y un job propio en CI. Tokens de marca fijos (F7.2): la paleta del prototipo —cuerpo claro, obsidiana y dorado para header, footer, hero y paneles—, las tres tipografías de §3.5.6 auto-hospedadas en WOFF2 (18 archivos, 324 KB, solo los subconjuntos `latin` y `latin-ext`), escala tipográfica fluida con `clamp` y escala de espacios. `BaseLayout.astro` (F7.3) con canonical, `hreflang` EN/ES, Open Graph, Twitter Card, JSON-LD de la empresa, enlace de salto al contenido y preload de las dos fuentes del primer pantallazo — Lighthouse SEO 100/100 corriendo en CI. Header completo (F7.4): barra superior con selector EN/ES, medallón oficial recortado en PNG circular con fondo transparente, mega menú de ancho completo que abre por CSS (sin depender del script), barra compacta al bajar y menú móvil a pantalla completa con acordeones — el móvil es un `<details>`, así que abre y cierra sin una línea de JavaScript. Footer (F7.5) con teléfono, WhatsApp, oficinas y políticas **leídos de `company_settings` en el build** por el endpoint público nuevo `GET /catalog/company`; sin backend a la vista, cae al placeholder marcado de D-P4. Home (F7.6) completa: las 26 secciones que se publican del prototipo (27 menos testimonios, D-P3), extraídas por `scripts/extract-home.mjs`, con cuatro composiciones propias — tarjetas de zona y de flota con datos reales de CTC, rejilla de servicios a sangre con seis fichas enlazadas a rutas reales, y el FAQ en `<details>` nativos. Ninguna afirmación de la prueba social de la referencia se publica. Hero con video diferido (F7.7) y animaciones — reveal, Ken Burns, brillo dorado, barra de progreso (F7.8) — como CSS e `IntersectionObserver` en una isla de 604 bytes. `/arrival-guide` (F7.9) migrada de sus cinco secciones, sin ningún `<video>` (D-P6): solo el poster propio del prototipo hasta que el cliente entregue el suyo. |
...
| **Tests del sitio** | ✅ 58 pruebas de Playwright en Chromium real, contra el sitio construido (no el dev server): HTML sin depender de JS, la isla de React hidratando, las tres tipografías cargando de verdad y servidas del propio sitio, canonical y `hreflang`, JSON-LD, enlace de salto, mega menú con mouse y con teclado, Escape, header compacto al bajar, menú móvil a pantalla completa medido contra el viewport, los flotantes sin tapar CTAs ni el footer, todos los destinos en el HTML con el JS apagado, el teléfono a E.164, que no se publique ningún `tel:` ni `wa.me` mientras el contacto sea provisional, las 26 secciones de la home con sus tarjetas de zona/flota y rejilla de servicios, el hero (poster LCP, video diferido, `reduced motion`, móvil), el reveal/brillo/barra de progreso de F7.8, y `/arrival-guide` (F7.9: sus cinco secciones, cero `<video>`, CTA en una sola línea, sin JS, sin desbordamiento móvil). Más Lighthouse SEO en 100/100 y `npm run perf` (LCP, presupuesto de 2.5 s) ahora corriendo contra `/` y `/arrival-guide`, todo en CI. |
...
F7  Sitio público: migrar prototipo [█████-----] 8/16
...
- [x] **F7.2** — `styles/tokens.css` con la paleta de `site/luxe.css` (obsidiana y dorado champagne), las tipografías nuevas de §3.5.6 (Cormorant Garamond, Cinzel y Manrope) y la escala de espacios. Verificar: Marlon aprueba dos muestras lado a lado antes de fijar los tokens. Las dos muestras se le presentaron (A, la de §3.5.6, y B, una alternativa de dos familias con Bodoni Moda); **no eligió**, así que quedó fija la A, que es la ya acordada. Cambiar a B son tres líneas de `tokens.css` más una corrida de `scripts/fonts.mjs`. Verificado en Chromium: las tres familias se aplican y **cargan de verdad** (no caen al respaldo en silencio), se sirven del propio sitio y no de Google, y la paleta llega a la página. **Corregido al empezar F7.6:** los tokens habían asumido un sitio oscuro de arriba abajo, y el prototipo aprobado es **claro debajo del hero** (obsidiana y dorado quedan para header, footer, hero y paneles). Se agregaron las superficies claras medidas del prototipo y la escala tipográfica se alineó a la suya, para que portarlo sección por sección no cambie tamaños.
...
- [x] **F7.6** — Home por secciones (componentes) idéntica al prototipo, con textos listos para reescribir (F9.1). Verificar: diff visual con capturas de 1440 y 400 px. Las 26 secciones que se publican del prototipo (27 menos testimonios, D-P3) tienen su contenido real —títulos, párrafos con énfasis, listas, fotos, 20 preguntas del FAQ y los CTA—, extraído por `scripts/extract-home.mjs`. Cuatro secciones recuperaron su composición propia: **tarjetas de zona** (foto, número de zona, filete dorado, precio grande y tiempo de viaje, con los diez nombres de §3.5.6 y las tarifas del catálogo, refrescadas contra la API cuando hay backend), **fichas de flota** (capacidades y 'desde' del catálogo; el prototipo las dibujaba con JavaScript, así que no había nada que portar), **rejilla de servicios** (seis fichas a sangre con foto, título y CTA, enlazadas a los destinos reales del mega menú en vez de los `href="#"` del prototipo) y el **FAQ** en `<details>` nativos. **La sección de testimonios no se publica** y se quitaron cinco afirmaciones de prueba social de la referencia ("5.0 Google reviews (901)", "since 2013", "#1 in Cabo"): son cifras de All Ways y CTC no las tiene — D-P3. Vuelve en F16.1. Verificado con 38 pruebas de Chromium contra el build real, capturas a 1440 px comparadas con el prototipo, y Lighthouse SEO 100/100. El cotizador del primer bloque es F8 y el hero con video, F7.7.
- [x] **F7.7** — Hero: poster como LCP y video diferido (WebM y MP4 ligeros), versión móvil, `prefers-reduced-motion`. Verificar: LCP < 2.5 s en Lighthouse móvil. Verificado: `npm run perf` (Lighthouse, throttling real de DevTools) mide **1.9 s**, con margen — el poster sale de `astro:assets` en cinco anchos (480-2000 px, WebP) para no mandarle a un teléfono los 2000 px del original, y el `<link rel=preload>` comparte exactamente esos mismos anchos (`imagesrcset`) para no bajar el archivo dos veces. El video no lleva `<source>` en el HTML — se los agrega un script tras `requestIdleCallback`, y nunca si `prefers-reduced-motion` está activo — y solo la pareja ligera (`hero-promo-light.mp4/webm`, ~6 MB); la pareja de escritorio de mayor resolución es F7.15, porque ni siquiera ella cabe en su propio presupuesto de §12.1 sin recodificarse. 8 pruebas nuevas de Chromium; `npm run perf` se sumó a CI.
- [x] **F7.8** — Portar animaciones (reveal, Ken Burns, brillo dorado, barra de progreso) como CSS e IntersectionObserver en una isla mínima. Verificar: sin animaciones con reduced motion; JS < 5 KB. El Ken Burns ya había quedado hecho en F7.7 (el poster del hero). `Motion.astro` (una sola isla, incluida una vez en `BaseLayout`, no por página) trae las otras dos: reveal por `[data-reveal]` con `IntersectionObserver` (se marcan todas visibles de una vez si hay menos movimiento o el navegador no soporta `IntersectionObserver`) y la barra de progreso, que no depende de reduced motion porque no es más que un indicador atado al scroll. Sin JavaScript, un `<noscript>` en `BaseLayout` cancela el `opacity: 0` de `[data-reveal]` — nada queda oculto para quien no ejecuta el script. El brillo dorado es puro CSS (`.ctc-shine`, sin JS) en los dos CTA sólidos del sitio (el del hero y el primero de cada sección). Verificado: el script mide **604 bytes** minificado (criterio: < 5 KB), y 6 pruebas de Chromium cubren el reveal en pantalla, todo visible de una vez con menos movimiento, todo visible sin JavaScript, y la barra de progreso moviéndose con el scroll.
- [x] **F7.9** — `/arrival-guide` migrada del prototipo (D-P6: el video de la referencia muestra a *su* anfitriona y *su* letrero — el prototipo ya lo había resuelto sin ningún `<video>`, solo un poster propio, así que aquí no hay nada que filtrar, solo portar; el video real del cliente queda pendiente y reemplaza ese recuadro sin tocar el resto). Verificar: página sin marcas de terceros. Cinco secciones (hero, "cómo encontrarnos", aviso, cuatro pasos numerados + recuadro destacado, CTA final), extraídas por `scripts/extract-arrival-guide.mjs`. A diferencia de la home, cada sección tiene una forma distinta, así que el extractor usa un objeto con nombre por sección en vez del molde genérico de `extract-home.mjs`. Lo común entre los dos —limpiar entidades, texto con o sin `<strong>/<em>`, separar `<body>` en `<section>`— se movió a `scripts/lib/prototype-html.mjs`. El aviso "Be Aware" del prototipo usa un acento ámbar ajeno a la paleta de CTC; se reconstruyó con los mismos tokens de dorado en vez de sumar un segundo color de alerta (§3.5.6, "gastar el atrevimiento en un solo lugar"). El link interno a "dónde está Cabo" apuntaba a `href="#"` en el prototipo — página de F9 que no existe todavía — se dejó como texto plano en vez de publicar un enlace muerto.
  Portar esta página destapó dos bugs reales en el extractor de la home (silenciosos hasta ahora, corregidos en el mismo commit): (1) `<p[^>]*>` sin límite de palabra hacía match con el arranque de `<path>` de íconos SVG, colando texto duplicado o ajeno en varios párrafos — corregido a `<p\b[^>]*>`/`<li\b[^>]*>` en ambos extractores; (2) el corte de secciones por `<section` nunca acotaba el último bloque, así que absorbía el `<footer>` completo — en la home quedaba encubierto porque el FAQ (última sección) ya descartaba párrafos sueltos, pero en `arrival-guide.html` (donde la última sección es el CTA, sin ese filtro) el pie de página se colaba entero como si fuera contenido; corregido acotando a `body.lastIndexOf("<footer")` — el *último*, no el primero, porque cada cita de la sección de testimonios trae su propio `<blockquote><footer>`. Al re-extraer la home con las dos correcciones, `home.json` cambió en un solo punto (el FAQ perdió una CTA y una imagen que en realidad eran del pie de página) — verificado sección por sección contra la versión anterior, sin otras diferencias.
  Un bug de layout real: el CTA dorado final envolvía su texto en dos líneas y lo cortaba a la mitad ("RESE / RVE"), porque llevaba `inline-block` (a diferencia del CTA del hero y los de `HomeSection`, que son `<a>` simples) — combinado con `overflow: hidden` de `.ctc-shine` (F7.8), la caja colapsaba a un ancho menor que su propio texto. Se quitó `inline-block`, igual que en los demás CTA dorados.
  Nota para quien depure visualmente con capturas de Playwright en esta máquina: el Chromium headless por default renderizó el hero completo —foto y, en un momento, hasta el de la home— como negro sólido, con los pixeles exactos de `--color-deep`, pese a que el HTML, el CSS computado y el archivo de imagen servido eran correctos (confirmado muestreando el `<img>` cargado por `canvas` desde dentro de la página, que sí mostraba sus colores reales). Era un bug de rasterizado GPU del sandbox, no del sitio: lanzando Chromium con `--disable-gpu --force-color-profile=srgb` la foto aparece bien. Ninguna captura de este tipo debe tomarse como diagnóstico sin ese flag.
  Verificado con 6 pruebas nuevas de Chromium (estructura de las cinco secciones, cero `<video>` en la página, el CTA en una sola línea, contenido completo sin JavaScript, poster listo para LCP sin depender de JS, sin desbordamiento horizontal a 390 px) — 58 pruebas en total. `npm run perf` ahora revisa `/` y `/arrival-guide` (antes solo la home).
