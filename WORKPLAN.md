...
| Indicador | Estado |
|---|---|
| **Fase actual** | F2 — Motor de precios y catálogo: 10/12 (F2.10 y F2.11 igualan a All Ways). Falta F2.9 (Marlon revisa el ADR-002) y F2.12 (chofer por hora y activity transfers como servicios cotizables). F3 — Reservas públicas: 11/13 (F3.12 y F3.13 muy avanzadas; falta lo que depende de Stripe y de F2.12). F4 — Pagos con Stripe: 2/10 (F4.1, F4.9). F1 ✅ 12/12. F0 ✅ 9/9. Vista previa del prototipo en https://cabotransportationconcierge.vercel.app (noindex). |
...
**Siguiente tarea:** F4.2 y F4.3 (crear y confirmar el pago con Stripe), que ya pueden cobrar los montos correctos (con IVA o el depósito en efectivo). F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales como reserva) quedan para cuando se necesiten esos servicios. Para F4.10 hacen falta las llaves de prueba de Stripe del cliente.
