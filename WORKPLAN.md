...
| Indicador | Estado |
|---|---|
| **Fase actual** | F4 — Pagos con Stripe: 5/10 (F4.1, F4.2, F4.3, F4.4, F4.9): el webhook ya respalda la confirmación rápida si el navegador no la termina. F2 — Motor de precios y catálogo: 10/12 (falta F2.9, revisión de Marlon, y F2.12, chofer por hora y activity transfers). F3 — Reservas públicas: 11/13 (falta lo que depende de F2.12). F1 ✅ 12/12. F0 ✅ 9/9. Vista previa del prototipo en https://cabotransportationconcierge.vercel.app (noindex). |
...
**Siguiente tarea:** F4.6 a F4.8 (link de pago del admin, reembolsos, pagos manuales) esperan al admin (F6); F4.5 (encolar el correo al pasar a `PAID`) espera al worker de correos (F5). F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales como reserva) quedan para cuando se necesiten esos servicios. Para F4.10 hacen falta las llaves de prueba de Stripe del cliente. Conviene pasar a F5 o F6 para desbloquear lo que queda de F4.
