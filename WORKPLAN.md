...
| Indicador | Estado |
|---|---|
| **Fase actual** | F6 — Auth y API del admin: ✅ 13/13 (login, sesión, TOTP obligatorio para owner/manager, CSRF de doble token, listado y acciones de reservas, alta manual, despacho, flota y cuentas por cobrar, tareas, auditoría automática, dashboard y KPIs, catálogo CRUD, usuarios y roles con lista de auditoría). F5 — Emails, PDF y trabajos: 8/11 (falta lo programado: recordatorio 24 h, reseña, webhooks de Resend). F4 — Pagos con Stripe: 6/10 (F4.1 a F4.5, F4.9). F2 — Motor de precios y catálogo: 10/12 (falta F2.9, revisión de Marlon, y F2.12). F3 — Reservas públicas: 11/13. F1 ✅ 12/12. F0 ✅ 9/9. Vista previa del prototipo en https://cabotransportationconcierge.vercel.app (noindex). |
...
**Siguiente tarea:** F6 (auth y API del admin) quedó completo, 13/13, y F5.9 (aviso al chofer asignado) ya está resuelto sobre el despacho de F6.7. Lo que queda de F5 (F5.7 recordatorio 24 h, F5.8 solicitud de reseña, F5.11 webhooks de Resend) pide una tarea programada, todavía sin diseñar. F4.6 (link de pago real de Stripe Checkout) es lo único que falta de pagos manuales/reembolsos, ya cubiertos por F6.5. F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales) quedan para cuando se necesiten esos servicios. Para F4.10 hacen falta las llaves de prueba de Stripe del cliente. Con el backend del admin completo, F7 (migrar el prototipo del sitio público a Astro) es la siguiente fase grande del WORKPLAN.
