...
- [ ] **F7.6** — Home por secciones (componentes) idéntica al prototipo, con textos listos para reescribir (F9.1). Verificar: diff visual con capturas de 1440 y 400 px. **En curso, primera tanda hecha:** las 27 secciones del prototipo ya están en la página con su contenido real —títulos, párrafos con su énfasis, listas, 33 fotos, 20 preguntas del FAQ y los CTA—, extraído por `scripts/extract-home.mjs` en vez de transcrito a mano. **Segunda tanda: las tarjetas de zona** ya tienen la composición del prototipo (foto a sangre, número de zona, filete dorado, precio grande y tiempo de viaje) rellenada con los datos propios de CTC: los diez nombres de §3.5.6 y las tarifas del catálogo, que el build refresca contra la API cuando hay backend. Las **fichas de flota** también, con las capacidades y el 'desde' del catálogo (el prototipo las dibujaba con JavaScript: su HTML estático no traía ninguna). **La sección de testimonios no se publica** y se quitaron cuatro afirmaciones más de prueba social de la referencia ("5.0 Google reviews (901)", "since 2013", "#1 in Cabo"): son cifras de All Ways y CTC no las tiene — D-P3 ya decía "datos reales del cliente o se quita la sección". Vuelve en F16.1. **Falta** la rejilla de servicios. El cotizador del primer bloque es F8 y el hero con video, F7.7.
