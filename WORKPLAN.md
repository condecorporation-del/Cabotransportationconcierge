...
**Siguiente tarea:** F6 (auth y API del admin) quedó completo, 13/13; F5.9 (aviso al chofer) y F4.6 (link de pago real de Stripe Checkout) también. De F4 solo faltan F4.8 (recibo del pago manual como documento) y F4.10 (prueba real con Stripe CLI, bloqueada por las llaves de prueba del cliente). De F5 falta lo que pide una tarea programada, todavía sin diseñar: F5.7 (recordatorio 24 h), F5.8 (solicitud de reseña) y F5.11 (webhooks de Resend). F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales) quedan para cuando se necesiten esos servicios. Con el backend del admin completo, F7 (migrar el prototipo del sitio público a Astro) es la siguiente fase grande del WORKPLAN.
...
- [ ] **F4.8** — Pagos manuales (efectivo, transferencia, cuenta) con recibo. Verificar: test. El registro del pago manual ya existe (`POST /admin/bookings/{id}/mark-paid`, F6.5: efectivo, transferencia, cuenta) y queda auditado; falta el recibo como documento (PDF o similar) — no se ha construido, distinto del voucher de la reserva.
