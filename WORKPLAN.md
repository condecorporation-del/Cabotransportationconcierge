...
| Indicador | Estado |
|---|---|
| **Fase actual** | F4 — Pagos con Stripe: 4/10 (F4.1, F4.2, F4.3, F4.9): ya se puede crear el intent y confirmar el pago. F2 — Motor de precios y catálogo: 10/12 (falta F2.9, revisión de Marlon, y F2.12, chofer por hora y activity transfers). F3 — Reservas públicas: 11/13 (falta lo que depende del webhook y de F2.12). F1 ✅ 12/12. F0 ✅ 9/9. Vista previa del prototipo en https://cabotransportationconcierge.vercel.app (noindex). |
...
**Siguiente tarea:** F4.4 (webhook de Stripe, la fuente de verdad si falla la confirmación rápida) y F4.5 (encolar el correo al pasar a `PAID`, aunque el envío real espera a F5). F2.12 y el resto de F3.12 (chofer por hora, city tour y traslados locales como reserva) quedan para cuando se necesiten esos servicios. Para F4.10 hacen falta las llaves de prueba de Stripe del cliente.
